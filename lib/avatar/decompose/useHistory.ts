 ...
 *
 * Memory cost: each snapshot is a raw `ImageData` (4 bytes/pixel).
 * For a 1024² source that's 4 MB; with 30 entries each touching one
 * canvas, the worst case is ~120 MB. Typical layer regions are far
 * smaller (~ 256×512 = 0.5 MB → 15 MB for 30 actions).
 */
...
export interface HistoryEntry {
  ...
  snapshots: ReadonlyMap<CanvasKey, ImageData>;
}
...
export interface UseHistoryResult {
  ...
  setBaseline: (snapshots: ReadonlyMap<CanvasKey, ImageData>) => void;
...
export function useHistory(): UseHistoryResult {
   ...
   *  fallback. */
  const computeState = useCallback((ptr: number): Map<CanvasKey, ImageData> => {
    ...
    const result = new Map<CanvasKey, ImageData>();
  ...
  const setBaseline = useCallback((snapshots: ReadonlyMap<CanvasKey, ImageData>) => {
  ...
  const commit = useCallback((label: string, changes: ReadonlyMap<CanvasKey, ImageData>) => {
  ...
  const undo = useCallback((): Map<CanvasKey, ImageData> | null => {
  ...
  const redo = useCallback((): Map<CanvasKey, ImageData> | null => {
  ...
  const goto = useCallback(
    (index: number): Map<CanvasKey, ImageData> | null => {
