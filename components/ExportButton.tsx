...
type Props = {
  ...
  /** Live runtime adapter — required for "export model file" because
   *  the bake step asks the adapter for pristine atlas bitmaps and
   *  per-layer triangles. `null` while the puppet is still loading. */
...
/**
 * Two file-download actions in the editor header. Both write a zip to
 * the user's downloads folder — they don't touch the IndexedDB
 * library, and they don't push to Geny. Geny sync happens
 * automatically through `lib/sync/genySync` whenever the library
 * (IDB) is written; these buttons exist only for users who want a
 * standalone copy on disk.
 *
 *   • **save file** — build a `*.geny-avatar.zip` containing the
 *     original bundle + sidecar variant rows + per-layer mask /
 *     AI-texture blobs + visibility map. Re-importing it brings the
 *     editor back to the exact session state, including all variants.
 *     The atlases inside this zip are pristine; our editor knows how
 *     to reapply the sidecar overrides at load time.
 *
 *   • **export model file** — build a runtime-ready puppet zip whose
 *     atlas PNGs have the user's edits *baked in*. AI textures are
 *     composited, mask-erased pixels are wiped, and explicitly-hidden
 *     layers' atlas footprints are erased so any third-party Spine /
 *     Cubism viewer renders the puppet the way our editor showed it.
 *     No Variants survive — this is one frozen state, not a session.
 *
...
export function ExportButton({ puppetId, adapter, className = "" }: Props) {
  ...
  const saveTitle = puppetId
    ? `Download a .geny-avatar.zip file — re-importable session (variants + overrides${
        totalLoadBearing > 0
  ...
  const modelTitle = !puppetId
    ...
    : !adapter || !avatar
      ...
      : `Download a runtime-ready .zip file — atlas + model patches baked in${
          totalLoadBearing > 0
